...
public class Node implements NodeInterface {
    ...
    private void handleWrite(String txID, String body, InetAddress addr, int port) throws Exception {
        ...
        // Route to correct store based on key type
        boolean isAddressKey = key.startsWith("N:");
        boolean hasKey = isAddressKey ? addressStore.containsKey(key) : dataStore.containsKey(key);
        boolean isClose = isOneOfThreeClosest(key);

        String response;
        if (hasKey) {
            if (isAddressKey) addressStore.put(key, value);
    ...
    // Actively query known nodes to discover more peers closer to targetHash
    private void discoverNodes(byte[] targetHash) throws Exception {
        // Convert hash to hex for the N message
        StringBuilder hex = new StringBuilder();
        for (byte b : targetHash) hex.append(String.format("%02x", b));
        String hashHex = hex.toString();

        // Query all currently known nodes
        List<String> toQuery = new ArrayList<>(addressStore.keySet());
        for (String node : toQuery) {
            ...
            String response = sendRequest(txID + " N " + hashHex, ip, port);
            if (response == null || !response.startsWith("O ")) continue;

            // Parse O response: pairs of encoded nodeName + encoded address
            String rest = response.substring(2);
            while (!rest.isEmpty()) {
                int[] nameBounds = parseCRNStringLen(rest);
                if (nameBounds == null) break;
                String name = rest.substring(nameBounds[0], nameBounds[1]);
                rest = rest.substring(nameBounds[2]);
                int[] addrBounds = parseCRNStringLen(rest);
                if (addrBounds == null) break;
                String address = rest.substring(addrBounds[0], addrBounds[1]);
                rest = rest.substring(addrBounds[2]);
                if (name.startsWith("N:") && !addressStore.containsKey(name)) {
    ...
    public String read(String key) throws Exception {
        byte[] keyHash = HashID.computeHashID(key);
        discoverNodes(keyHash); // actively find closer peers first
        List<String> closest = getClosestNodes(keyHash, 3);
